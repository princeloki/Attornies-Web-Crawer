...
def parser(url):
    try:
        ...
        with codecs.open(r'Files/attorneys.csv','a',encoding='utf-8') as f:
...
def main():
    with open(r'Files/attorneys.csv', 'w+') as f:
        fieldnames = ['Name','Block Number','Profile Url']
        thewriter = csv.DictWriter(f,fieldnames)
        thewriter.writeheader()
    print 'Running..............'
    begin = time.time()
    pages = get_pages('http://members.calbar.ca.gov/search/ls_results.aspx?county=Los+Angeles&specialty=08')
    pool = ThreadPool(4)
